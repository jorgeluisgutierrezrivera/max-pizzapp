...
class TimbreDePrueba implements Timbre {
  ...
  @override
  Future<void> habilitar() async => activo = true;
  @override
  void sonar() => sonidos++;
}
...
Map<String, dynamic> json(int id,
        {String estado = 'pendiente', int minutos = 5, String cliente = 'Ana Prueba', String? observacion}) =>
    {
      'id': id,
      'estado': estado,
      ...
      'lineas': [
        {
          ...
          'cantidad': 2,
          'extras': [
            {'producto': {'id': 20, 'nombre': 'Extra queso'}, 'cantidad': 2},
          ],
        },
        {
...
final usuario = Usuario.desdeJson({'sub': 'x', 'nombre': 'Cocina Demo', 'usuario': 'cocina.demo', 'roles': ['cocina']});
...
void main() {
  group('los estados de la pantalla', () {
    ...
    testWidgets('sin pedidos lo dice, en vez de una pantalla en blanco', (t) async {
      await abrir(t, []);
      expect(find.text('No hay pedidos en cocina.\nLos nuevos aparecen aquí solos, en orden de llegada.'), findsOneWidget);
    ...
    testWidgets('si la API falla, el mensaje y Reintentar', (t) async {
      ...
      await t.pumpWidget(MaterialApp(
        ...
        home: PantallaCocina(
          ...
          cambiarEstado: (id, hacia) => throw UnimplementedError(),
          crearCanal: () => canal,
  ...
  group('lo que muestra cada pedido', () {
    testWidgets('número, cliente, para llevar, las pizzas con mitades y extras, las bebidas aparte y la observación', (t) async {
      await abrir(t, [json(12, observacion: 'Sin cebolla')]);
      expect(enTarjeta(12, find.text('#12')), findsOneWidget);
    ...
    testWidgets('en orden de llegada: el que más espera va primero', (t) async {
      ...
      final x = [for (final id in [1, 2, 3]) t.getTopLeft(find.byKey(Key('pedido-$id'))).dx];
  ...
  group('empezar y marcar listo', () {
    ...
    testWidgets('si otro lo cambió antes (409), avisa y relee la cola', (t) async {
      ...
      await t.tap(enTarjeta(7, find.text('Empezar')));
      await t.pumpAndSettle();
      expect(find.text('El pedido #7 ya había cambiado. La cola se actualizó.'), findsOneWidget);
  ...
  group('en vivo', () {
    ...
    testWidgets('si recepción lo cancela, sale de la cola y cocina se entera', (t) async {
      ...
      expect(find.text('Recepción canceló el pedido #3 de Luis Prueba.'), findsOneWidget);
