...
// (test/soporte/emisor.js) que fabrica los tokens que Keycloak no emite a voluntad.
...
test.before(async () => {
  ...
  const montarExtra = (rutas, { exigirRol }) => {
    rutas.get('/prueba/solo-cocina', autenticar, exigirRol('cocina'), (req, res) => res.json({ ok: true }));
  };

  const poolSano = { query: async () => ({ rows: [{ '?column?': 1 }] }) };
  const poolCaido = { query: async () => { throw new Error('connect ECONNREFUSED'); } };

  api = await levantarApp(crearApp({ pool: poolSano, autenticar, montarExtra }));
  apiSinBd = await levantarApp(crearApp({ pool: poolCaido, autenticar }));
  base = api.base;
  baseSinBd = apiSinBd.base;
});
